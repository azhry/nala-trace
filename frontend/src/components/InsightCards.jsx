...
function ImprovementActions({ ledger, followUps = [] }) {
  ...
  const legacyImprovements = ledger?.legacyImprovements || []
  const hasActions = improvements.length > 0
  const hasLegacyActions = legacyImprovements.length > 0
  const hasFollowUps = followUps.length > 0
  const recordedLabel = `${improvements.length.toLocaleString()} valid agent ${improvements.length === 1 ? 'action' : 'actions'}`
  ...
  const legacyLabel = `${legacyImprovements.length.toLocaleString()} legacy ${legacyImprovements.length === 1 ? 'target' : 'targets'} excluded`
  const statusLabel = [hasActions && `${improvements.length.toLocaleString()} valid`, hasFollowUps && followUpStatusLabel, hasLegacyActions && legacyLabel].filter(Boolean).join(' · ')

  return <section className={`analysis-block analysis-actions-block ${hasActions || hasFollowUps ? 'has-actions' : 'is-empty'}`} aria-labelledby="analysis-actions-title"><div className="analysis-block-heading"><div><span className="section-label">Action plan</span><h3 id="analysis-actions-title">What to improve</h3><p>Use this plan to change agent behavior or its instruction Markdown. The project field is context only, never an improvement target.</p></div><StatusPill tone={hasActions || hasFollowUps ? 'positive' : hasLegacyActions ? 'warning' : 'unknown'}>{statusLabel || 'No actions recorded'}</StatusPill></div>{ledger?.project && <div className="analysis-ledger-context"><span>Project context</span><strong>{ledger.project}</strong><p>This identifies the evaluated project; it is not a valid improvement target.</p></div>}{hasActions && <div className="analysis-actions-group"><div className="analysis-actions-group-heading"><strong>Recorded changes</strong><span>{recordedLabel}</span></div><ol className="analysis-actions-list">{improvements.map((improvement, index) => <li key={`${improvement.target}-${index}`}><div className="analysis-action-field"><span>Change</span><strong>{improvement.change}</strong></div><div className="analysis-action-field"><span>Target</span><strong>{improvement.targetLabel}</strong>{improvement.path && <code>{improvement.path}</code>}</div><div className="analysis-action-field"><span>Why</span><p>{improvement.reason}</p></div></li>)}</ol></div>}{hasFollowUps && <div className="analysis-actions-group analysis-derived-actions"><div className="analysis-actions-group-heading"><strong>Follow-ups to address</strong><span>{followUpLabel}</span></div><ol className="analysis-actions-list">{followUps.map((followUp, index) => <li key={`${followUp.signalKey}-${index}`}><div className="analysis-action-field"><span>Finding</span><strong>{followUp.title}</strong></div><div className="analysis-action-field"><span>Next step</span><strong>{followUp.action}</strong></div><div className="analysis-action-field"><span>Why this is here</span><p>{followUp.reason}</p></div>{followUp.occurrenceCount != null && <div className="analysis-action-field"><span>Recorded occurrences</span><span>{followUp.occurrenceCount.toLocaleString()}</span></div>}<span className="analysis-action-source">Derived from warning finding</span></li>)}</ol></div>}{hasLegacyActions && <div className="analysis-actions-group analysis-derived-actions analysis-legacy-actions"><div className="analysis-actions-group-heading"><strong>Legacy targets excluded</strong><span>{legacyLabel}</span></div><p className="analysis-action-legacy-note">Older evaluations may point at project source. These records remain visible for traceability, but they are excluded from the agent action plan.</p><ol className="analysis-actions-list">{legacyImprovements.map((improvement, index) => <li key={`${improvement.target}-${index}`}><div className="analysis-action-field"><span>Target</span><strong>{improvement.targetLabel}</strong>{improvement.path && <code>{improvement.path}</code>}</div><div className="analysis-action-field"><span>Status</span><strong>Excluded from agent improvements</strong></div><div className="analysis-action-field"><span>Recorded change</span><strong>{improvement.change}</strong></div><div className="analysis-action-field"><span>Why excluded</span><p>{improvement.outOfScopeReason}</p></div></li>)}</ol></div>}{!hasActions && !hasFollowUps && !hasLegacyActions && <div className="analysis-inline-empty"><strong>No improvement actions recorded</strong><span>This evaluation did not record a concrete change to make.</span></div>}{!hasActions && !hasFollowUps && hasLegacyActions && <div className="analysis-inline-empty"><strong>No valid agent improvement actions recorded</strong><span>The saved evaluation contains only legacy targets outside the agent and instruction Markdown scope.</span></div>}</section>
}
