...
function EvidenceRecord({ category, record }) {
  ...
  return <li className="analysis-evidence-record"><div className="analysis-evidence-record-heading"><strong>{label}</strong><code>{identifier}</code></div><div className="analysis-evidence-tags"><StatusPill tone={record.necessary === 'no' ? 'critical' : record.necessary === 'yes' ? 'positive' : 'unknown'}>Necessary for task: {evidenceValue(record.necessary)}</StatusPill></div>{category.key === 'tools' && <><div className="analysis-evidence-input"><span>{record.inputPreviewLabel || 'Tool details'}</span><code>{record.inputPreview || 'Tool input not recorded'}</code></div><div className="analysis-evidence-context"><span>Completion evidence</span><p>{record.completionDetail}</p></div></>}{category.key === 'skills' && <div className="analysis-evidence-context"><span>Captured invocation</span><code>{record.invocationDetail}</code></div>}<div className="analysis-evidence-context"><span>Decision basis</span><p>{record.rationale}</p></div></li>
}
...
function ImprovementActions({ ledger, followUps = [] }) {
  ...
  return <section className={`analysis-block analysis-actions-block ${hasActions || hasFollowUps ? 'has-actions' : 'is-empty'}`} aria-labelledby="analysis-actions-title"><div className="analysis-block-heading"><div><span className="section-label">Evaluator actions</span><h3 id="analysis-actions-title">What to improve</h3><p>Stored evaluator actions plus concrete next steps derived from recorded warning findings.</p></div><StatusPill tone={hasActions || hasFollowUps ? 'positive' : 'unknown'}>{[hasActions && `${improvements.length.toLocaleString()} recorded`, hasFollowUps && followUpStatusLabel].filter(Boolean).join(' · ') || 'No actions recorded'}</StatusPill></div>{hasActions && <div className="analysis-actions-group"><div className="analysis-actions-group-heading"><strong>Recorded evaluator actions</strong><span>{recordedLabel}</span></div><ol className="analysis-actions-list">{improvements.map((improvement, index) => <li key={`${improvement.path}-${index}`}><div className="analysis-action-field"><span>Path</span><code>{improvement.path}</code></div><div className="analysis-action-field"><span>Concrete change</span><strong>{improvement.change}</strong></div><div className="analysis-action-field"><span>Reason</span><p>{improvement.reason}</p></div></li>)}</ol></div>}{hasFollowUps && <div className="analysis-actions-group analysis-derived-actions"><div className="analysis-actions-group-heading"><strong>Follow-ups from recorded findings</strong><span>{followUpLabel}</span></div><ol className="analysis-actions-list">{followUps.map((followUp, index) => <li key={`${followUp.signalKey}-${index}`}><div className="analysis-action-field"><span>Finding</span><strong>{followUp.title}</strong></div><div className="analysis-action-field"><span>Next step</span><strong>{followUp.action}</strong></div><div className="analysis-action-field"><span>Why it is listed</span><p>{followUp.reason}</p></div>{followUp.occurrenceCount != null && <div className="analysis-action-field"><span>Recorded occurrences</span><span>{followUp.occurrenceCount.toLocaleString()}</span></div>}<span className="analysis-action-source">Derived from recorded warning</span></li>)}</ol></div>}{!hasActions && !hasFollowUps && <div className="analysis-inline-empty"><strong>No improvement actions recorded</strong><span>This evaluation did not record a concrete change to make.</span></div>}</section>
}
...
function AnalysisCards({ analysis, compact = false, sessionId }) {
  ...
  return <aside className={`insights-section analysis-insights analysis-full-page ${analysis.recorded ? '' : 'is-unrecorded'}`} aria-labelledby="analysis-details-title"><div className="section-heading-row analysis-heading"><div><p className="section-label">Evaluation review</p><h2 id="analysis-details-title">Recorded evaluation evidence</h2><p className="section-description">Stored decisions, evaluator results, and improvement actions for this session.</p></div><StatusPill tone={analysis.recorded ? 'positive' : 'unknown'}>{analysis.recorded ? 'Recorded' : 'Not recorded'}</StatusPill></div>{analysis.recorded ? <div className="analysis-content">{analysis.evaluation.recorded && <ImprovementActions ledger={analysis.evaluation.evaluationLedger} followUps={analysis.evaluation.followUps} />}<AnnotationPanel annotation={analysis.annotation} /><EvaluationPanel evaluation={analysis.evaluation} /><p className="analysis-provenance">{[analysis.annotation.source && `annotation: ${analysis.annotation.source}`, analysis.evaluation.source && `evaluation: ${analysis.evaluation.source}`, formatUpdatedAt(analysis.updatedAt)].filter(Boolean).join(' · ') || 'Analysis provenance not recorded'}</p></div> : <NoAnalysisState />}</aside>
}
