...
export default function ToolCallCard({ event, defaultOpen = false, selected = false }) {
  ...
  return (
    <article id={`trace-event-${encodeURIComponent(String(event.id || 'unknown'))}`} data-trace-event-id={event.id} data-trace-event-selected={selected ? 'true' : 'false'} tabIndex={selected ? -1 : undefined} className={`tool-card ${open ? 'is-open' : ''} ${event.status === 'attention' ? 'has-attention' : ''} ${selected ? 'is-evidence-selected' : ''}`}>
      <button
        type="button"
